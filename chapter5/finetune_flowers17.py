...
(X_train, X_test, y_train, y_test) = train_test_split(data, labels, train_size=0.25, random_state=42)
...
model.fit_generator(aug.flow(X_train, y_train, batch_size=1), validation_data=(X_test, y_test),
epochs = 25, steps_per_epoch = len(X_train)//1, verbose=1)
...
model.fit_generator(aug.flow(X_train, y_train, batch_size=1), validation_data=(X_train, y_train), epochs=100, steps_per_epoch = len(X_train)//1, verbose=1)
